...
// Type traits to determine publisher type based on node type
template <typename NodeT>
struct DiagnosticsPublisherTraits
{
  using PublisherType = rclcpp::Publisher<diagnostic_msgs::msg::DiagnosticArray>;
  using PublisherPtr = typename PublisherType::SharedPtr;
  static constexpr bool is_agnocast = false;
};

template <>
struct DiagnosticsPublisherTraits<agnocast::Node>
{
  using PublisherType = agnocast::Publisher<diagnostic_msgs::msg::DiagnosticArray>;
  using PublisherPtr = typename PublisherType::SharedPtr;
...
public:
  ...
  void publish(const rclcpp::Time & publish_time_stamp) override
  {
    if constexpr (Traits::is_agnocast) {
      ...
      *msg = create_diagnostics_array(publish_time_stamp);
